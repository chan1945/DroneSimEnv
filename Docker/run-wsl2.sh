...
COMPOSE_FILE="${SCRIPT_DIR}/Dockerfile/docker-compose-wsl2.yml"
...
# 이 스크립트는 WSL2 전용이므로 실제 WSL 환경에서 실행 중인지 확인합니다.
ensure_wsl2() {
    if ! grep -qi "microsoft" /proc/version; then
        echo "ERROR: This script is for WSL2 environment only." >&2
        exit 1
    fi
}

# WSL2에서 새 터미널 창과 한글/이모지 출력을 위해 필요한 패키지를 확인하고,
# 없으면 자동 설치합니다.
ensure_wsl2_dependencies() {
    if command -v xfce4-terminal >/dev/null 2>&1 && dpkg -s fonts-noto-cjk >/dev/null 2>&1; then
        return
    fi

    echo "The terminal app (xfce4) or Korean/emoji fonts are not installed."
    ...
    echo "Required WSL2 terminal packages and fonts are ready."
}

# WSL2 터미널에서 한글/이모지 출력이 깨지지 않도록 UTF-8을 강제합니다.
configure_locale() {
...
# xfce4-terminal 새 창을 열고, 지정한 컨테이너에 bash로 접속합니다.
open_terminal() {
    local title="$1"
    local service="$2"
    local message="$3"
    ...
    env GTK_THEME=Adwaita:dark xfce4-terminal \
        --title="${title}" \
        -x bash -c "echo '${message}'; docker exec -it ${service} /bin/bash" &
}
...
command_up() {
    ...
    # WSLg 환경에서는 실패해도 compose 실행 자체는 계속할 수 있게 경고만 출력합니다.
